...
1. JBoss BPM Suite from 6.1 release onwards does not bundle EAP inside its installer. Visit the link:#{site.base_url}/products/eap/download/[EAP downloads page] to download and then install EAP version 6.4 before using the JBoss BPM Suite installer.
2. Visit the link:#{site.base_url}/products/bpmsuite/download[JBoss BPM Suite download page] and download the installer.
3. Make sure you have a JDK of your choice installed. We recommend OpenJDK or Oracle JDK.
4. You can now (optionally) install the Integration Stack for JBoss Developer Studio. This provides IDE support for developing JBoss BPM Suite applications. For more details, see the link:/products/devstudio/get-started/#ExtraSection[installation instructions].
